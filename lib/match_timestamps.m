...
%         sync_shots.wm_blue = data.wm.blue_freq(this_wm_idx);
        ...
        sync_shots.probe_set(this_lv_idx) = 2*sync_shots.wm_setpt(this_lv_idx) - opts.aom_freq;
    ...
    wm_msr_mask = (abs(wm_set_err) < opts.wm_tolerance)';
    ...
    [cal_hist,cal_bin_edges]= histcounts(sync_cal.N_atoms,opts.num_cal_bins);
    cal_bin_cents = 0.5*(cal_bin_edges(1:end-1)+cal_bin_edges(2:end));
    
    [wm_hist,cal_wm_bin_edges] = histcounts(wm_set_err(wm_msr_mask),opts.num_cal_bins);
